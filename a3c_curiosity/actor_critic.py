...
class ActorCritic(nn.Module):
    ...
    def calc_cost(self, new_state, hx, done, rewards, values, log_probs, intrinsic_rewards=None):
        returns = self.calc_R(done, rewards, values)
        if intrinsic_rewards is not None:
            returns += intrinsic_rewards.detach().numpy() # detach from graph, don't want to backprop through intrinsic rewards
